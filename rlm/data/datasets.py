...
def get_project_folders(base_path: str = "data/projects") -> Dict[str, Dict[str, Any]]:
    """
    Scan for project folders containing documents.

    Returns:
        Dict with folder info, keyed by numeric string IDs starting from 1
    """
    ...
    for i, folder in enumerate(sorted(base.iterdir()), start=1):
        if folder.is_dir() and not folder.name.startswith("."):
            ...
            if file_count > 0:
                folders[str(i)] = {
                    "name": f"📁 {folder.name} ({file_count} files)",
                    "path": str(folder),
                    "type": "folder",
...
def get_all_datasets() -> Dict[str, Dict[str, Any]]:
    """
    Get all available project folders.

    Returns:
        Dict of project folders
    """
